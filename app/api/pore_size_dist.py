...
async def compute_pore_size_dist(
    ...
    # 3. Determine output filenames dynamically from the input file's stem
    base_output_filename = f"{input_path.stem}.psd"
    final_output_filename = f"{input_path.stem}.psd_histo"

    # 4. Build the arguments list
    zeo_args = [
        "-psd",
        str(effective_chan_radius),
        str(probe_radius),
        str(samples),
        base_output_filename # Pass base name, even if it might be ignored
    ]
    if ha:
        zeo_args.insert(0, "-ha")

    logger.info(f"[{task_name}] Running Zeo++ with args: {' '.join(zeo_args)} {input_path.name}")
    
    # 5. Run the command
    result = runner.run_command(
        structure_file=input_path,
        zeo_args=zeo_args,
        output_files=[final_output_filename], # We expect the dynamically named _histo file
    ...
    # 7. Check for and parse the output file
    ...
    # 8. Construct and return the successful response
